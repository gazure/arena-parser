...
impl ScryfallDataManager {
    ...
    fn get_cached_card(&self, card_id: i32) -> anyhow::Result<Option<(CardType, Card)>> {
        let mut statement = self.conn.prepare(
            "SELECT card_type, card_json FROM cards WHERE id = ?1",
        )?;
    ...
    pub fn get_card_info(&self, card_id: i32, cards_database: &CardsDatabase) -> anyhow::Result<(CardType, Card)> {
    ...
    fn fetch_card_info(&self, card_id: i32) -> anyhow::Result<(CardType, Card)> {
        ...
        let card = Card {
            name: resp_json["name"].as_str().unwrap_or(&card_id_str).to_string(),
            mana_value,
            quantity: 1,
        };
        let card_type = resp_json["type_line"].as_str().unwrap_or("");
